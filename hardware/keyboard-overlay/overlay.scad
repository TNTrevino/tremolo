/*
 * Tremolo keyboard overlay — piano prototype (v3, piano orientation).
 *
 ...
 *
 * Black keys are flush with the whites, not raised: the part prints
 * top-face-down and a raised black top cannot lie on the bed. Position,
 * short bodies near the base, and engraved labels carry the piano look.
 *
...
// How far the white press zone reaches toward the player past the home
// row (it hovers over the flats row and the spacebar).
white_front = 40;
...
module neck(x0, w) {
	...
	translate([x0 + (w - nw) / 2, -flexure_length,
	...
	difference() {
		...
		if (cut_left > 0)
			translate([-0.1, -0.1, -0.1])
				cube([cut_left + 0.1, black_clear_end + 0.1,
					key_thickness + 0.2]);
		if (cut_right > 0)
...
//   print = upside down on the bed: rotate 180° about x. The key tops
//           lie on the bed, nothing overhangs, the flexures print as
//           clean layers. Flip the print over and set the rail toward
//           the screen; that lands exactly on the use orientation.
if (orient == "use")
	translate([key_count * key_pitch, lever_length, 0])
		rotate([0, 0, 180])
			overlay();
else
	translate([0, lever_length, key_thickness])
		rotate([180, 0, 0])
			overlay();
