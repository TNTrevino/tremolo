...
/**
 * Interval Identification: shows two notes (stacked or sequential); the
 * player names the interval -- quality + size, or size only when Require
 * Quality is off.
 *
 * Port of
 * frontend-react/src/features/identification-game/games/interval.ts, which
 * was already a `.ts`: nothing here changes but `fetchQuestion`'s shape.
 */
