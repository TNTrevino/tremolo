...
describe("DashboardPageComponent", () => {
	...
	it("renders the profile's statistics in the stat grid", async () => {
		...
		expect(text).toContain("12"); // total sessions, from total_entries
		// 12 sessions x 5 assumed minutes = 60 minutes = "1h 0m".
		expect(text).toContain("1h 0m");
	});

	/**
	 * Pins the gap rather than hiding it. `general-info` sends no
	 * `average_npm` and no `average_accuracy` -- no endpoint does -- so both
	 * tiles read zero, exactly as React's have always read zero. If a later
	 * phase finds a real source for either, this spec is what fails and says
	 * so.
	 */
	it("shows zero for the two statistics the service never sends", async () => {
		await renderStudent();

		const text = el().textContent ?? "";
		expect(text).toContain("0%"); // average accuracy -- no wire source
		expect(text).not.toContain("47.2");
		expect(text).not.toContain("91.5");
	});

	it("renders the join date the service pre-formatted, not a parsed one", async () => {
		await renderStudent();

		// React parsed a `created_at` that never arrives and rendered
		// "Joined Invalid Date". The Go service sends the formatted string.
		expect(el().textContent).toContain("Joined 15 Jan 2026");
