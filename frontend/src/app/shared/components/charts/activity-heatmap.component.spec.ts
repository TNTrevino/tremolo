...
describe("ActivityHeatmapComponent", () => {
	...
	async function render(data: DailyActivityCount[] = []): Promise<void> {
	...
	it("colours a day with games differently from an empty one", async () => {
		await render([{ date: dateOffset(1), game_count: 9 }]);
	...
	it("ramps busy days up the colour scale, thresholds and all", async () => {
		await render([
			{ date: dateOffset(1), game_count: 1 },
			{ date: dateOffset(2), game_count: 3 },
			{ date: dateOffset(3), game_count: 6 },
			{ date: dateOffset(4), game_count: 20 },
		]);
	...
	it("describes the hovered day, and clears when the pointer leaves", async () => {
		await render([{ date: dateOffset(1), game_count: 1 }]);
